...
def mergeMasks(target, conv):
  ...
  target.paste(Image.new('RGB', (320, 512), (255,255,255)), (0,0), new_img)
  return target
...
predictions = os.listdir(result_path)
iou_avg = 0
precision_avg = 0
recall_avg = 0
accuracy_avg = 0
...
for pred in predictions:
  mergedFolder = result_path + '/Merged/'
  convertedFolder = result_path + "/Converted/"
  ...
  image_nr_ind = pred.find('_')
  image_nr = pred[:image_nr_ind]

  test_mask = test_folder + '/' + image_nr + '_mask.tif'
  print(image_nr)
  img = cv2.imread(result_path + '/' + pred, cv2.IMREAD_GRAYSCALE)
  ...
  # save
  converted_name = image_nr + '_conv.jpg'
  cv2.imwrite(convertedFolder + converted_name, mask)

  # Merged images
  mask_path = test_folder + '/' + image_nr + '_mask.tif'
  conv_path = convertedFolder + converted_name
  mergeMasks(mask_path, conv_path).save(mergedFolder + image_nr + '_merged' + '.png')
